...
def init_db():
    ...
    try:
        cursor.execute("ALTER TABLE gmail_stock ADD COLUMN used_by INTEGER DEFAULT NULL")
...
def get_user(user_id):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, balance, today_tasks, total_tasks, referred_by, referrals_count, is_active FROM users WHERE user_id = ?", (user_id,))
...
def add_user(user_id, referred_by=None):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("INSERT OR IGNORE INTO users (user_id, balance, today_tasks, total_tasks, referred_by, referrals_count, is_active) VALUES (?, 0.0, 0, 0, ?, 0, 0)", (user_id, referred_by))
    conn.commit()
    conn.close()
...
async def myid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🆔 **আপনার টেলিগ্রাম আইডি হলো:** `{user_id}`", parse_mode="Markdown")
...
async def check_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "check_join":
        ...
        try:
            member = await context.bot.get_chat_member(chat_id=chat_username, user_id=user_id)
            if member.status in ['member', 'administrator', 'creator']:
                ...
                await query.message.reply_text(
                    ...
                    "দয়া করে আগে গ্রুপে জয়েন করুন, তারপর **'✅ জয়েন সম্পন্ন করেছি'** বাটনে চাপ দিন।",
                    parse_mode="Markdown",
                    reply_markup=join_keyboard
                )
        except Exception:
            await query.message.reply_text(
                ...
                "নিশ্চিত করুন বটটিকে আপনার সাপোর্ট গ্রুপে **অ্যাডমিন (Admin)** হিসেবে যুক্ত করা আছে।",
                parse_mode="Markdown",
                reply_markup=join_keyboard
            )
...
async def clear_gmail_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id != ADMIN_ID:
    ...
    await update.message.reply_text("🗑️ **জিমেইল স্টকের সমস্ত ডাটা সফলভাবে ক্লিয়ার করা হয়েছে!**", parse_mode="Markdown")

async def stock_status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id != ADMIN_ID:
        await update.message.reply_text("⛔ আপনার অ্যাডমিন অ্যাক্সেস নেই।")
        return
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM gmail_stock WHERE status = 'available'")
    available = cursor.fetchone()[0]
    
    cursor.execute("SELECT COUNT(*) FROM gmail_stock WHERE status = 'used' OR status = 'approved'")
...
async def get_used_gmails(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT email, password, used_by FROM gmail_stock WHERE status = 'used' OR status = 'approved'")
    rows = cursor.fetchall()
    conn.close()

    if not rows:
        await update.message.reply_text("⚠️ **কোনো ব্যবহৃত জিমেইল পাওয়া যায়নি!**")
        return
    ...
    await update.message.reply_document(
        document=file_bytes,
        caption=f"📂 **ইউজারদের সম্পন্ন করা {len(rows)} টি জিমেইল এর ফাইল।**",
        parse_mode="Markdown"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_withdraw_wallet"):
        ...
        if balance < MIN_WITHDRAW:
            await update.message.reply_text(f"❌ আপনার পর্যাপ্ত ব্যালেন্স নেই। সর্বনিম্ন উইথড্র ৳{MIN_WITHDRAW}")
        ...
        cursor.execute("INSERT INTO withdrawals (user_id, amount, method, wallet, status) VALUES (?, ?, ?, ?, 'pending')", 
        ...
        try:
            await context.bot.send_message(
                chat_id=ADMIN_ID,
                text=f"💸 **নতুন উইথড্র রিকোয়েস্ট এসেছে!**\n\n"
                     f"👤 **ইউজার আইডি:** `{user_id}`\n"
                     f"💰 **অ্যামাউন্ট:** ৳{balance:.2f}\n"
                     f"📌 **মাধ্যম:** {method}\n"
                     f"📱 **বিকাশ/নম্বর (কপি করতে টাচ করুন):**\n`{wallet}`\n\n"
                     f"⚠️ *আগে ইউজারকে টাকা পাঠান, তারপর 'Approve' চাপুন।*",
                parse_mode="Markdown",
    ...
    if user_id == ADMIN_ID and ("@gmail.com" in text or text.startswith("/bulkadd")):
        ...
        while i < len(tokens):
            if "@" in tokens[i]:
                ...
                try:
                    cursor.execute("INSERT INTO gmail_stock (email, password) VALUES (?, ?)", (email, password))
                    added_count += 1
                except Exception:
                    failed_count += 1
            else:
    ...
    if "ব্যালেন্স & উইথড্র" in text or "ব্যালেন্স" in text:
        ...
        withdraw_keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("💸 টাকা উইথড্র করুন", callback_data="request_withdraw")]
        ])
        await update.message.reply_text(balance_text, parse_mode="Markdown", reply_markup=withdraw_keyboard)
        ...
        if gmail:
            ...
            task_text = (
                ...
                "ধাপ ১: উপরের ইমেইল ও পাসওয়ার্ড দিয়ে নতুন অ্যাকাউন্ট তৈরি করে কাজ সম্পন্ন করুন।\n"
            ...
            await update.message.reply_text("⚠️ **দুঃখিত, এই মুহূর্তে জিমেইল স্টক খালি আছে!**\nকিছুক্ষণ পর আবার চেষ্টা করুন।")
        ...
        rules_text = (
            ...
            "১. দয়া করে বট থেকে জিমেইল এবং পাসওয়ার্ড নিয়ে সঠিক নিয়মে জিমেইল একাউন্ট ক্রিয়েট করুন।\n\n"
            "⚠️ **বিশেষ নোটিশ (অবশ্যই পালনীয়):**\n"
            "📌 **জিমেইল খোলার পরই ফোন থেকে অ্যাকাউন্টটি Log Out / Remove করে দিবেন।** অন্যথায় পেমেন্ট পাবেন না।\n"
        ...
        ref_text = (
            ...
            "আপনার রেফারেল লিংকের মাধ্যমে বন্ধুকে ইনভাইট করুন। আপনার রেফারে আইডি এক্টিভ হলে পাবেন **১০ টাকা** বোনাস!\n\n"
            f"🔗 **আপনার রেফার লিংক:**\n`{ref_link}`"
        )
        ...
        if WORK_VIDEO_LINK.strip():
            video_keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("▶️ ভিডিওটি দেখতে এখানে ক্লিক করুন", url=WORK_VIDEO_LINK)]
            ])
            await update.message.reply_text(
            ...
            await update.message.reply_text(
                "🎥 **খুব শীঘ্রই কাজের ভিডিও আসছে!**\n\nভিডিও আপলোড হওয়া মাত্রই আপনারা এখানে দেখতে পাবেন। আপাতত নিয়ম পড়ে কাজ চালু রাখুন।",
                parse_mode="Markdown"
            )

    elif "হেল্পলাইন" in text:
        help_keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("💬 এডমিন সাপোর্ট", url=f"https://t.me/{HELPLINE_USERNAME}")]
        ])
        help_text = (
            "🆘 **এডমিন হেল্পলাইন:**\n\n"
            "যে কোনো প্রয়োজনে বা সমস্যার কারণে আমাদের এডমিনের সঙ্গে যোগাযোগ করুন।\n\n"
            "⚠️ **বিশেষ নোটিশ:** অযথা কেউ মেসেজ দিবেন না।"
        )
        await update.message.reply_text(help_text, parse_mode="Markdown", reply_markup=help_keyboard)
...
async def main_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "request_withdraw":
        ...
        if balance < MIN_WITHDRAW:
            await query.message.reply_text(f"❌ আপনার পর্যাপ্ত ব্যালেন্স নেই। উইথড্র করতে কমপক্ষে **৳{MIN_WITHDRAW}** প্রয়োজন। আপনার বর্তমান ব্যালেন্স: ৳{balance:.2f}")
            return
        ...
        await query.message.reply_text(
            f"💳 **আপনার বর্তমান ব্যালেন্স: ৳{balance:.2f}**\n\nপেমেন্ট নেওয়ার জন্য নিচের যেকোনো একটি মাধ্যম সিলেক্ট করুন:",
            parse_mode="Markdown",
        ...
        prompt_text = f"📱 আপনার **{method}** নম্বরটি বা ওয়ালেট অ্যাড্রেসটি লিখে পাঠান:"
        if method == "USDT":
        ...
        await query.message.edit_text(
            ...
            f"আপনার জিমেইলের **{int(GMAIL_PRICE)} টাকা** ২৪ ঘন্টার মধ্যে পেয়ে যাবেন এবং জিমেইলটি বর্তমানে প্রসেসিং এ দেওয়া হয়েছে।",
            parse_mode="Markdown"
        )
        
        admin_keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"✅ কাজ অ্যাপ্রুভ ({int(GMAIL_PRICE)} টাকা যোগ)", callback_data=f"approve_{user_id}_{gmail_id}")],
            [InlineKeyboardButton("❌ বাতিল করুন", callback_data=f"reject_{user_id}_{gmail_id}")]
        ])

        if gmail_info:
            email, password = gmail_info
            admin_msg = (
                f"📥 **নতুন টাস্ক জমা পড়েছে!**\n\n"
                f"👤 **ইউজার আইডি:** `{user_id}`\n\n"
                f"🔍 **চেক করার জন্য তথ্য (কপি করতে টাচ করুন):**\n"
                f"📧 **ইমেইল:**\n`{email}`\n"
                f"🔑 **পাসওয়ার্ড:**\n`{password}`\n\n"
                f"⚠️ *আগে ফায়ারফক্স ফোকাস ব্রাউজারে সাইন-ইন করে চেক করুন। ঠিক থাকলে অ্যাপ্রুভ করুন।* "
            )
        else:
            admin_msg = f"📥 **নতুন টাস্ক জমা পড়েছে!**\n\n**ইউজার আইডি:** `{user_id}`\n**জিমেইল আইডি:** `{gmail_id}`"
...
async def admin_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve_"):
        ...
        cursor.execute("SELECT referred_by, is_active FROM users WHERE user_id = ?", (target_user_id,))
        res = cursor.fetchone()
        
        cursor.execute("UPDATE users SET balance = balance + ?, today_tasks = today_tasks + 1, total_tasks = total_tasks + 1 WHERE user_id = ?", (GMAIL_PRICE, target_user_id))
        
        if res and res[0] and res[1] == 0:
            referred_by = res[0]
            cursor.execute("UPDATE users SET is_active = 1 WHERE user_id = ?", (target_user_id,))
            cursor.execute("UPDATE users SET balance = balance + 10, referrals_count = referrals_count + 1 WHERE user_id = ?", (referred_by,))
            try:
                await context.bot.send_message(chat_id=referred_by, text="🎉 **আপনার রেফারকৃত ইউজার কাজ শেষ করায় আপনার একাউন্টে ১০ টাকা কমিশন যোগ হয়েছে!**", parse_mode="Markdown")
            except Exception:
                pass
        ...
        await query.message.edit_text(f"✅ User `{target_user_id}` এর কাজ সফলভাবে অ্যাপ্রুভ করা হয়েছে এবং ১৬ টাকা ব্যালেন্সে যোগ হয়েছে।")
        try:
            await context.bot.send_message(chat_id=target_user_id, text=f"🎉 **আপনার জমা দেওয়া জিমেইলটি অ্যাপ্রুভ করা হয়েছে!**\nআপনার ব্যালেন্সে {int(GMAIL_PRICE)} টাকা যোগ করা হয়েছে।", parse_mode="Markdown")
        except Exception:
            pass
        ...
        await query.message.edit_text(f"❌ User `{target_user_id}` এর কাজ রিজেক্ট করা হয়েছে।")
        try:
            await context.bot.send_message(chat_id=target_user_id, text="❌ **আপনার জমা দেওয়া কাজটি বাতিল করা হয়েছে।**", parse_mode="Markdown")
        except Exception:
            pass
        ...
        try:
            await context.bot.send_message(chat_id=target_user_id, text=f"🎉 **আপনার উইথড্র রিকোয়েস্ট সফলভাবে পেমেন্ট করা হয়েছে!**\n💰 অ্যামাউন্ট: ৳{amount:.2f}", parse_mode="Markdown")
        except Exception:
            pass
        ...
        await query.message.edit_text(f"❌ ইউজার `{target_user_id}` এর উইথড্র রিজেক্ট করা হয়েছে এবং ব্যালেন্স রিফান্ড করা হয়েছে।")
        try:
            await context.bot.send_message(chat_id=target_user_id, text=f"❌ **আপনার উইথড্র রিকোয়েস্টটি বাতিল করা হয়েছে এবং ৳{amount:.2f} আপনার ব্যালেন্সে ফিরিয়ে দেওয়া হয়েছে।**", parse_mode="Markdown")
        except Exception:
            pass
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_action_callback, pattern="^(approve_|reject_|w_approve_|w_reject_)"))
